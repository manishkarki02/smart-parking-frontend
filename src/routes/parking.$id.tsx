...
function ParkingDetailsPage() {
  ...
  const handleBookNow = () => {
    if (!selectedSlot && location?.availableSlots && location.availableSlots > 0) {
  ...
  const handleLoginSuccess = () => {
    // Already set false by modal, now navigate to new booking seamlessly
    navigate({
      to: "/bookings/new",
      search: { parkingLocationId: location?.id },
    });
            ...
            <Card>
              <CardContent className="pt-6">
                ...
                <div className="space-y-4 mt-6">
                  ...
                  <div className="flex items-start text-muted-foreground">
                    <div className="w-5 h-5 mr-3 mt-0.5 font-bold text-primary">$</div>
                    <div>
                  ...
                  {location.distance && (
                     <div className="flex items-start text-muted-foreground">
                       ...
                       <div>
                         ...
                         <p className="text-sm">{location.distance.toFixed(1)} km from current place</p>
                       </div>
                     </div>
                  )}
                  ...
                  <div className="pt-4 mt-4 border-t">
                     <p className="text-sm text-foreground/80 leading-relaxed">
                        Secure and affordable parking space provided by {location.vendorName}. 
                        Conveniently located with easy access. Total of {location.availableSlots} slots available right now.
                     </p>
                  </div>
                </div>
              </CardContent>
             ...
             <div className="bg-card rounded-2xl border p-6 shadow-sm h-full">
                ...
                {location.availableSlots > 0 ? (
                  ...
                  <div className="p-8 text-center bg-red-50 text-red-600 rounded-xl border border-red-200">
                    ...
                    <p className="text-sm mt-1">There are no available slots at this location right now.</p>
                  </div>
                )}
                ...
                <div className="mt-8 pt-6 border-t flex items-center justify-between">
                   <div>
                     {selectedSlot ? (
                       ...
                       <p className="text-muted-foreground text-sm">Select an available slot to continue</p>
                     )}
                   </div>
                   <Button 
                     size="lg" 
                     ...
                     disabled={location.availableSlots === 0 || (!selectedSlot && location.availableSlots > 0)}
                   >
                     Book Now
                   </Button>
                </div>
